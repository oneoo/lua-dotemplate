...
function dotemplate(f, is_return, init)
	...
	if _cache[_f] then return pcall(_cache[_f]) end
	...
	for i = 1,len,2 do
		if inb == false and html:byte(i) == bs then -- {{
			...
			inb = true
			has_q = false
		elseif _bs > 0 and html:byte(i) == be then -- }}
			local _be = 0
			...
			inb = false
		elseif inb == false and html:byte(i) == bbb then
	...
	local codes, err = loadstring(concat(_codes), f)
	if not codes then return nil, err end
	_cache['h'] = true
	_cache[_f] = codes
	
	local _r, _e = pcall(codes)
	if is_return then
		if _r then
			_r = concat(_e)
			_e = nil
